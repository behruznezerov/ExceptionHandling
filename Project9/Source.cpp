...
public:
		...
		if (user_count > 0)
		{
			for (int i = 0; i < user_count; i++)
			{
				if (username == users[i]->GetUsername()) {
					...
					throw DatabaseException("Bele Istifadeci Yoxdur", __LINE__, "GetUserByUsername");
			}
		}
		else
		{
	...
	void DeleteUserById(const int& id) {
		if (id < user_count + 1 && user_count != 0) {
			...
			throw InvalidArgumentException("Bele ID Yoxdur", __LINE__, "DeleteUserById");
	}
...
public:
	...
	void SignIn(string username, string password) {
		if (_database.FindUserByUsername(username)) {
			...
			if (temp.GetPassword() == password)
			{
				...
				throw DatabaseException("Password Sehvdir", __LINE__, "SignIn");
		}
		else {
	...
	void SignUp(string username, string password, string name, string surname) {
		if (_database.FindUserByUsername(username)) {
			throw DatabaseException("Username Yoxdur", __LINE__, "SignUp");
		}
		if (username.length() < 6)
		{
			throw InvalidArgumentException("Username  sayi 6dan balaca  ola bilmez", __LINE__, "SignUp");
		}
		if (username[0] != toupper(username[0])) {
			throw InvalidArgumentException("Usernamenin ilk herfi boyuk olmalidir", __LINE__, "SignUp");
		}
		if (password.length() < 6) {
			throw InvalidArgumentException("Passwordun  sayi 6dan balaca ola bilmez", __LINE__, "SignUp");
		}
		if (name.length() < 4) {
			throw InvalidArgumentException("Name  sayi 4den balaca ola bilmez!", __LINE__, "SignUp");
		}
		if (surname.length() < 4) {
			throw InvalidArgumentException("Surname  sayi 4den balaca ola bilmez ", __LINE__, "SignUp");
		}
	}
};
...
public:
	static void Control() {
		try {
			Database db;
			User Kenan("kenan12", "Eskerov", "Kenan", "esker252");
			Registration facebook(db);
			facebook.getDatabase().AddUser(Kenan);
			facebook.SignIn("Kenan12", "eskerov252");
		}
		catch (Exception& ex) {	
			ex.Message();
		}
